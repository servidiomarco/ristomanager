 ...
 *
 * Interamente idempotente (IF NOT EXISTS / ON CONFLICT / DROP IF EXISTS):
 * su ristocomande è già girata una volta e la riga pgmigrations è stata
 * rimossa (1/09, per sbloccare il collaudo cassa) — lì ri-girerà sopra gli
 * oggetti esistenti.
 */
...
export const up = (pgm) => {
    ...
    pgm.sql(`
        CREATE TABLE IF NOT EXISTS sala_node_certs (
    ...
    // Il CHECK sui nomi feature va allargato PRIMA del seed. ATTENZIONE al
    // pattern: si riparte SEMPRE dall'ultima lista completa (qui quella di
    // modulo-asporto), mai da una vecchia — una lista monca fallirebbe sulle
    // righe esistenti e farebbe rollback dell'intera migration.
