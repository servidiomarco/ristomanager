...
- **Close buttons are a filled square target**, not a bare glyph: `h-9 w-9 rounded-[var(--ds-radius-control)] bg-[var(--ds-surface-row)]` with a 16px glyph, as `ModalShell` defines it. The point is the filled 36px target, not the shape — it was a circle until the radius scale was unified.
- **Three radii, and `rounded-full` is not one of them.** `--ds-radius` (6px) for boxes, `--ds-radius-control` (8px) for buttons/fields/pills/chips, `--ds-radius-sm` (4px) for what sits *inside* a box. `rounded-full` is reserved for things that are genuinely circles or stadiums rather than controls: avatars and initials, status dots, progress bars, sheet grab-handles, **toggle switch tracks and knobs** (the shape is the gesture), and the floating mobile bottom bar with its `+` and active-tab pill. Adding a `rounded-[16px]` back into a component is a regression — reach for a token.
