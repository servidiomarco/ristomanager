...
- **Close buttons are a filled target**, not a bare glyph: `h-9 w-9 rounded-[var(--ds-radius-control)] bg-[var(--ds-surface-row)]` with a 16px glyph, as `ModalShell` defines it. The point is the filled 36px target, not the shape — the token draws it as a circle or a squircle depending on the interface style.
- **Three radii, and `rounded-full` is not one of them.** `--ds-radius` for boxes, `--ds-radius-control` for buttons/fields/pills/chips, `--ds-radius-sm` for what sits *inside* a box. Their VALUES depend on the per-user interface style (`users.preferred_design_style`, card in Impostazioni → Profilo): the default «classico» is 16px boxes and full-pill controls — the app's historical look — while «squadrato» is 6/4/8px; `index.css` keeps classico on `:root` and squadrato under `:root[data-design='squadrato']`, applied by App.tsx. `rounded-full` is reserved for things that are genuinely circles or stadiums rather than controls: avatars and initials, status dots, progress bars, sheet grab-handles, **toggle switch tracks and knobs** (the shape is the gesture), and the floating mobile bottom bar with its `+` and active-tab pill. Adding a `rounded-[16px]` back into a component is a regression — reach for a token: writing controls through the token is the only thing that lets the two styles coexist.
- **A primitive that bakes `flex`/`inline-flex`/`grid` into its base class cannot be hidden with a plain `hidden` from the call site.** Both are unvariant utilities, so Tailwind's emit order decides, not the string order. Use `max-md:hidden`. See rule 15 in the design-system doc.
