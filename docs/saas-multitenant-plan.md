...
- **Aggiornamento (feat/rls-rigida)**: policy strict-capable ri-asserita a ogni boot (`ensureRlsPolicies`, copre anche le tabelle post-B4 senza policy); contesto tenant automatico via AsyncLocalStorage in `db.ts` (authenticate, webhook, scheduler, boot dichiarano tenant o piattaforma — nessun call site riscritto); ruolo applicativo non-superuser attivo in produzione. La modalità rigida si accende con `ALTER DATABASE railway SET app.rls_strict = 'on'` (reversibile con RESET, senza deploy) — certificata dall'intera suite (147 test) eseguita con ruolo non-superuser e strict acceso (`TEST_STRICT_RLS=1 npm test`). **`DEFAULT 1` rimosso** (migration drop-default-tenant-id): i 35 seed della baseline e le route preset dichiarano il tenant esplicito, e una INSERT senza tenant muore di NOT NULL invece di finire silenziosamente nel tenant 1.
