...
*Aggiornato il 4 settembre 2026. Ordine reale delle operazioni per passare scontrino e
...
- Token di produzione attivo su Railway (`OPENAPI_INVOICE_TOKEN` +
  `OPENAPI_INVOICE_BASE_URL=https://invoice.openapi.com`).
- IT-configuration di produzione ESISTENTE: `02423910781 · il vecchio
  frantoio s.r.l.` — fatture attive (`customer_invoice: true`), scontrini no.
- **E-Receipts NON è ancora attivabile in produzione** ("E-receipts service
  is not ready in PROD yet", risposta API del 04/09/2026): per gli scontrini
  si parte col binario classico qui sotto, o si aspetta l'apertura.

## 1b. Scontrini SUBITO: binario classico (documento commerciale online)

Il driver è commutabile via env — stessa app, stessi payload, stesso
downstream (numero/data, copia, registro, webhook):

1. Nella IT-configuration di produzione: `receipts: true` e
   `receipts_authentication` = credenziali Fisconline dell'esercente
   (codice fiscale, password, PIN — le stesse di "Fatture e Corrispettivi").
   Passaggio da fare col ristoratore/commercialista.
2. Su Railway: `OPENAPI_RECEIPTS_CHANNEL=receipts`.
3. Nota operativa: su questo binario l'emissione è ASINCRONA — il numero
   documento arriva pochi secondi dopo via callback (il webhook lo
   backfilla), e l'annullo è possibile solo a trasmissione avvenuta (un
   annullo immediato può rispondere "Receipt is not voidable": riprovare
   dopo qualche secondo).
4. Quando Openapi apre E-Receipts in produzione: attivarlo sulla
