...
|---|---|---|
| 2026-09-03 | Menu & Banchetti | Backend della gestione varianti: gruppi e opzioni creabili/modificabili dal CRM (i gruppi della cassa restano suoi per opzioni e massimi, ma rinomina, obbligo e interruttore sopravvivono agli import); gruppi assegnabili a qualsiasi piatto, anche quelli importati; piatti semplici/composti con ingredienti togliibili («Senza X», sconto configurabile); sovrapprezzi anche in percentuale del prezzo battuto; min/max delle scelte fatti rispettare dal server. UI in arrivo nella fase 2. |
| 2026-09-03 | Comande, Cucina e Passe | La barra dei piatti raggruppati in Cucina mostra solo ciò che deve ancora arrivare (uscite non chiamate comprese, anche di comande fuori schermo): un piatto sparisce dalla barra quando viene chiamato — il chiamato sta già sulle card. La pill delle note (label breve «Note») resta fissa a destra. |
