...
- Ordinamento per orario, nome, ospiti o data di creazione, in un **menu a tendina in cima al pannello dei filtri**: l'imbuto apre «Ordina per» e, sotto, «Filtra per». Contatori di sintesi; sezione Annullate separata.
...
- **Non propone mai una zona chiusa**: se per quel giorno le sale di una zona (di norma le esterne, fuori stagione) sono chiuse, Sofia non chiede «preferisce interno o esterno?» e non nomina le zone affatto — dice che c'è posto e prosegue, la prenotazione va nella zona aperta. Solo se è il cliente a chiedere la zona chiusa risponde «al momento le sale all'esterno sono chiuse» — mai «è tutto prenotato» o «non abbiamo posti», riservati alle zone aperte ma piene — e propone l'altra. Al cliente che insiste non promette che la zona chiusa possa liberarsi durante il servizio e non ne annota la preferenza: una sala chiusa resta chiusa tutto il giorno. La preferenza si annota invece quando la zona è aperta ma al completo. Se una nota chiede comunque una zona chiusa, la prenotazione arriva in sala con la smentita accanto, così nessuno legge un impegno impossibile.
...
- **Il nome sulla prenotazione è protetto dalla rubrica**: se il numero del chiamante è già registrato e il nome dettato non corrisponde al titolare, Sofia non salva alla cieca — chiede se la prenotazione è per il titolare del numero o per un'altra persona. Un nome detto a metà («Taddeo») viene completato col nome di rubrica («Taddeo Sergio»); una prenotazione intestata a una persona diversa porta in nota il titolare del numero. Le proposte WhatsApp approvate dallo staff passano senza domanda (l'approvazione vale come chiarimento) ma ricevono la stessa nota.
...
- **Board del giorno** (pagina Asporto, gruppo Servizio): ordini in lista per slot di ritiro, con ricerca per nome o telefono e filtro Attivi/Tutti; contatori attivi, da produrre, pronti, ritirati. Sul desktop **giorno e stop stanno nella testata dell'app**, col navigatore condiviso con Prenotazioni e Sale (frecce, calendario, «torna a oggi»); sul telefono stanno sulla riga sotto il titolo, con «Nuovo ordine» accanto al titolo. Lo stop resta sempre attaccato al giorno: ferma *quel* giorno. Un ordine nuovo si crea dal «+» della testata sul desktop, dal bottone in pagina sul telefono.
...
- **Impostazioni → Asporto**: **giorno della board** — «Indipendente» (il banco tiene il suo giorno, è il default) oppure «Segue l'app» (il giorno è quello della testata, lo stesso di prenotazioni, sala e cassa: si cambia una volta sola e vale per tutti); capienza per slot e minuti di preparazione; lo stop si mette e si toglie anche dalla board. Le manopole stanno sul permesso «Gestisce ordini asporto», non sulle impostazioni generali: sono decisioni di servizio.
...
- **Pagina pubblica `/ordina`** (e `/ordina/<slug>` per tenant): il cliente sceglie giorno e orario di ritiro (solo slot in cui la cucina fa in tempo), compone l'ordine dal menu Alla carta con quantità e note, lascia nome e telefono (obbligatorio) e riceve conferma immediata. Gate doppio come /prenota: modulo venduto **e** interruttore «Ordini online» acceso — spento, la pagina mostra la card di manutenzione coi contatti. Ordini col canale «WEB» in board, push allo staff alla creazione; honeypot e rate limit contro i bot; la pagina non può mai forzare capienza o stop. Bilingue come /prenota: selettore IT/EN in testata, lingua iniziale dal browser, date nel formato della lingua scelta.
- **Incasso al ritiro**: «Prepara il conto» chiude la comanda e mette il conto in coda Cassa — riga «Asporto HH:MM» col nome del cliente al posto del tavolo; da lì incasso, scontrino e libro cassa sono il flusso di sempre. La coda cassa funziona anche per chi ha il solo modulo asporto. Se il conto è ancora da incassare, «Ritirato» chiede una conferma esplicita (secondo tocco sul bottone): il sacchetto può uscire lo stesso, ma il conto aperto in cassa non passa inosservato.
- **Pagamento prima del ritiro**: dal pagamento in Cassa, «Invia link al cliente» manda il link del conto al telefono dell'ordine (WhatsApp con template dedicato se configurato, altrimenti SMS); il cliente paga da casa — e può girare il link agli amici, ognuno i propri piatti. Reinvio sempre possibile.
...
- **Titoli sempre in Title Case all'italiana**: nomi di piatti, categorie, varianti e ingredienti vengono normalizzati automaticamente («SPAGHETTI ALLE VONGOLE» → «Spaghetti alle Vongole», «filetto ai porcini» → «Filetto ai Porcini») sia al salvataggio dal CRM sia all'import dalla cassa — su tutte le superfici (menu, comande, cucina, cassa, menu pubblico, stampe). Preposizioni, articoli e congiunzioni restano minuscoli (elisioni comprese: «all'Aglio»); le denominazioni restano sigle: «barolo docg» → «Barolo DOCG» (DOC, DOCG, IGT, IGP, DOP, STG, AOC, AOP), puntate comprese («d.o.p.» → «D.O.P.»).
...
- **Vini abbinati**: nella modale «Categorie» una categoria si marca **Vino** (senza categorie vino valgono quelle Bar) e ogni scheda piatto ha la card «Vini abbinati» — fino a 3 vini della carta scelti a chip, con **«Suggerisci» (AI)** che propone e pre-seleziona: si salva solo col Salva della scheda. Dal QR modal, **«Abbina i vini»** fa proporre al sommelier AI gli abbinamenti per tutti i piatti ancora senza (idempotente, correggibile piatto per piatto), col suo interruttore di attivazione accanto.
...
- **Preventivo condivisibile**: ogni banchetto ha un link pubblico stabile (pagina senza login con menù per uscite, tariffe e totale, sempre aggiornata alle ultime modifiche) da inviare **su WhatsApp dal numero business del ristorante** (modello approvato da Meta; finché il modello non è attivo il canale si presenta «in attivazione») **o via email** (invio dal server con il pulsante «Apri il preventivo»). Le note operative interne non compaiono mai. La pagina è bilingue IT/EN (selettore in alto, lingua iniziale dal browser); i nomi dei piatti restano quelli del menù.
...
- **Sullo schermo largo Comande prende la pagina intera**: la testata dell'app lascia il posto a una barra della pagina con la ricerca tavolo, l'**imbuto** (solo il filtro per stato, con le pastiglie dei filtri di Prenotazioni e «Reimposta») e la pastiglia **Live** (ora e stato della connessione — prima si vedeva solo sui monitor di cucina). Anche la **sidebar si ritira**, in un bollo col marchio in alto a sinistra: il chevron la rimette accanto ai tavoli, che si stringono per farle posto. Comande lavora il servizio in corso — giorno e turno non si cambiano da qui, si cambiano dalla barra globale di un'altra pagina.
- **La griglia tavoli dice a che punto è ogni tavolo.** Le tessere si ordinano per numero di tavolo con le **sale in pista** sopra (Tutte, e una per sala col conteggio), e ognuna porta quattro righe: numero e pallino di stato, coperti, chi è al tavolo, e la riga di stato — «134,00 € · 2ª in cucina», «62,00 € · da incassare», l'ora per i prenotati, «libero». Prima il totale e lo stato dell'uscita si scoprivano aprendo il tavolo uno per uno. Le parole degli stati sono quelle delle pastiglie della comanda (in bozza, al passe, in cucina, pronta, servita): una lingua sola per la stessa informazione. Le comande rimaste aperte da un servizio passato non spariscono: la tessera le mostra come appese («39,00 € · appesa da ieri») in qualunque servizio si stia guardando, e toccarle riprende la comanda dal suo servizio vero — un tavolo con una comanda aperta non è mai «libero». Per vedere un solo stato — i conti da incassare, le comande aperte — c'è il filtro nell'imbuto.
- **Il menu sullo schermo largo è a schede**: le categorie sono una fila di schede tinte con **l'icona della categoria** (riconosciuta dal nome: gelato, pesce, griglia, vino…; forchetta quando non si riconosce) e il conteggio dei piatti battibili, e i piatti una griglia da **quattro per riga** con foto e stepper − / + sotto il prezzo. L'ordine della colonna è categorie → filetto → ricerca → piatti: la prima scelta è la categoria, e la ricerca sta appoggiata ai piatti che filtra. Lo stepper si aggiunge al tocco sulla scheda, che batte come sempre; il tocco sulla foto la apre a tutto schermo per mostrarla all'ospite. I piatti senza foto tengono il riquadro col glifo, così la griglia non balla riga per riga.
- **Dentro il tavolo, la barra della pagina porta il tavolo**: accanto alla freccia di ritorno stanno **Tav. N**, i coperti in una pastiglia, chi è al tavolo col totale (**«Senza prenotazione»** per i walk-in, così il vuoto è spiegato invece che assente), e a destra Conto e ⋮.
- **La colonna di destra è tutta della comanda**, in un pannello solo diviso da filetti: le tessere delle uscite in testa (nome e pallino di stato — verde se è partita, ambra quella in composizione, niente pallino se è vuota), sotto le uscite come sempre — pill del nome **a sinistra** sul bordo, spunta su quelle servite, quantità in pastiglia — e in fondo il solo bottone d'invio, che dice cosa manda e quanto vale: «Invia 2ª uscita · 36,00 €». «Invia tutto» resta, alla sua condizione di sempre (bozze fuori dall'uscita corrente). **La quantità di ogni riga porta la tinta della sua categoria**: un'uscita si legge per composizione — tre verdi e un rosa — prima di leggerne i nomi. Il totale sta nella barra della pagina accanto al tavolo; coperti, servizio e sconto nel foglio Conto, dove si va per incassare.
...
- **Layout «a pagine» a scelta dell'operatore** (Impostazioni → Profilo → «Comande sul palmare»): il palmare prende la geografia dell'app di cassa per chi ci ha la memoria muscolare — pagina delle categorie a righe grandi con ricerca in testa, dentro la categoria la lista piatti con la freccia di ritorno in accent, barra inferiore fissa Tavoli · Comanda · Menu (la Comanda si apre a tutta pagina, col pallino se ci sono bozze da inviare), uscita in composizione e bottone «Segue» sotto il pollice, col totale «da inviare» quieto a destra (righe e importo) che al tocco apre la Comanda. Anche la griglia tavoli si sfoglia per sala, con le linguette in basso (pallino sulle sale con comande aperte o conti da incassare) e le tessere in ordine di numero; la ricerca trova il tavolo in qualunque sala. Dentro il tavolo la testata è una riga sola — coperti, lente e menu ⋮ (il Conto sta lì) — la Comanda a tutta pagina dice in testa tavolo e cameriere che l'ha aperta, con le due letture (per uscita / tutto il tavolo) in un interruttore a icone accanto alla × piena in accent; e la pagina delle categorie si veste a scelta dell'operatore dal menu ⋮: lista a righe, o bottoni da 3 o 4 per riga. Nella vista a bottoni le categorie si dividono in sezioni vere — Cucina, Bar, Dolci, dalle stesse spunte del catalogo che forzano l'uscita — con intestazioni leggere (icona in cerchietto tinto, filetto, conteggio) e le tessere che entrano a cascata; senza spunte bar/dolci la griglia resta una, senza titoli inutili. Le tessere vestono come le schede categoria di Comande sullo schermo largo: tinta della categoria (lo stesso colore lì e qui), icona riconosciuta dal nome (gelato, pesce, vino…) e conteggio dei piatti battibili. Con l'uscita Bar in composizione le sezioni Cucina e Dolci si attenuano (e viceversa coi Dolci): un invito dell'occhio, non un divieto — le categorie restano toccabili. La logica non cambia: un tocco batte, le uscite sono le stesse, l'invio sta nella Comanda. La preferenza è per account e segue l'operatore su ogni palmare.
- **Uscite/portate**: ogni uscita è una card bordata con la sua pill del nome a cavallo del bordo (quella che si sta componendo in evidenza) e, quando è **in corso in cucina**, la card si accende — bordo pieno più spesso e fondo tinto. Finché una riga è in bozza si **sposta** su un'altra uscita — la singola riga col **tocco lungo sulla riga stessa** (niente più maniglia ⇅, che partiva da sola durante lo scroll): tenuta ferma la riga «si stacca» con vibrazione e anteprima, trascinata cade sull'uscita di arrivo, rilasciata senza muoversi apre il selettore; l'uscita intera resta sulla maniglia ⇕ a cavallo dell'angolo in alto a sinistra (tocco = selettore, trascinata = drag), con il bersaglio evidenziato in entrambe le prese. Dal selettore dell'uscita intera si può anche **eliminarla**: «Elimina la 2ª uscita» toglie tutte le sue righe non ancora inviate — con Annulla nel toast se erano solo sul palmare, con conferma al secondo tocco («Elimina 2 righe») se c'erano bozze già salvate sulla comanda. Le righe già inviate restano e si stornano dalla comanda. Sulla riga in bozza restano **matita e maniglia**: la matita (o il tocco sul nome) apre il **foglio di riga** — quantità, varianti, variante libera ("senza sale, metà porzione…"), peso ed «elimina riga» in un posto solo; coperti ±1.
- **Elimina la comanda** (menu ⋮ del tavolo): via la comanda intera — righe già in cucina comprese — finché non c'è un conto. Motivazione obbligatoria, registrata nel log attività; serve il permesso di storno, e le card spariscono anche dai monitor di cucina. La comanda di prova o aperta per sbaglio si toglie di mezzo senza stornare riga per riga.
- **Uscita Bar**: le categorie spuntate come «bar» in modale Categorie (bibite, vini, amari) hanno un'uscita dedicata in testa alla comanda — battendo un piatto di quelle categorie la riga ci va da sola, qualunque uscita sia selezionata, e un giro ripetuto ci torna. Il Bar è fuori dalla sequenza delle portate: nei lanci automatici parte subito insieme all'invio (anche in «prima uscita» e «a consumo»), senza rubare il turno alle uscite di cucina; su monitor e comanda stampata si legge «Bar». La riga si può comunque spostare fra Bar e uscite normali come ogni altra.
- **Uscita Dolci**: stessa meccanica del Bar per le categorie spuntate come «dolci» (dolci, gelati), ma dal verso opposto: l'uscita sta in coda alla comanda, dopo la 6ª. Un dolce battuto ci va da solo e nei lanci automatici parte subito con l'invio, senza chiamata dal passe; su palmare, monitor e comanda stampata si legge «Dolci».
- **Varianti a scala d'intensità** (foglio varianti, gruppi a scelta multipla): i tasti − e + muovono ogni variante su quattro gradini — «+ Nduja» (aggiunta, col sovrapprezzo), «Molta Nduja» (abbondante, stesso addebito dell'aggiunta), «Senza Nduja» (tolta, in sconto), «Poca Nduja» (gratis). Molta/Poca si accordano al nome («Molto/Poco Prosciutto»). Niente varianti-doppione a listino; etichetta e prezzo viaggiano identici su comanda, monitor cucina, preconto e scontrino. Le scelte singole (cotture) restano chip. Nel foglio ogni gruppo di varianti è una scheda sotto «Varianti»: la testata (nome, conteggio delle scelte, chevron) apre le opzioni dentro la stessa scheda, divise da hairline — fisarmonica a un'anta con l'animazione d'ingresso di casa; solo gli obbligatori stanno sempre aperti — il foglio si apre per la cottura, non per leggere ogni aggiunta possibile. A gruppo chiuso la testata dice cosa è scelto (il valore per le scelte singole, il conteggio per le multiple), e il bottone di conferma mostra il **prezzo vivo** del pezzo configurato — varianti, ingredienti tolti, peso e quantità compresi. Riaprendo una riga dalla comanda il foglio mostra anche l'uscita col chip per spostarla e «Aggiungi un altro», che duplica la riga com'è configurata nella sua uscita.
- **Il tap aggiunge, le varianti si scelgono dopo**: toccare un piatto lo batte subito anche se ha varianti facoltative — si scelgono poi dal foglio di riga (matita o tocco sul nome) o in anticipo col tocco lungo sul piatto — che su un piatto già battuto nell'uscita in composizione riapre la sua riga («Aggiorna»): la variante corregge il battuto, non lo duplica. Il foglio si apre al tocco solo dove serve per forza: piatti al peso e varianti obbligatorie (la cottura), e lì porta anche lo stepper quantità per battere più pezzi in un colpo. Su una riga da 2+ pezzi, scegliere nel foglio una quantità minore con varianti diverse **divide la riga**: la modifica va su quei pezzi, il resto rimane com'era — «2× filetto, uno al sangue» senza ribattere niente. Sotto la riga del piatto nel menu vivono le **sotto-righe delle combinazioni** («liscia», «+ Sale»…), ognuna col suo stepper − / + (a zero la riga si toglie; sul peso il + aggiunge un altro pezzo) e il tap che apre il foglio di quella riga — è lì che si differenzia («qty 1 + variante → Aggiorna» divide la riga). Compaiono da sole quando c'è struttura (più combinazioni, o varianti/nota/peso); per la sola riga liscia il cassetto si apre e richiude col **tap sul piatto**, e la freccina accanto al prezzo lo indica: punta giù da chiuso, si capovolge da aperto. A sotto-righe in vista la riga piatto si spoglia — restano nome, prezzo e «+»; anche da chiuse la riga tiene solo conteggio (il totale del piatto su tutte le uscite) e «+»: togliere vive nel cassetto, riga per riga, dove si vede quale combinazione si tocca. Sul palmare **si aggiunge solo dal «+»** — il tap sul nome non batte, così il doppio tap non crea equivoci (sulla griglia larga il tap batte come sempre). Ogni sotto-riga apre con il **chip della sua uscita**: un tocco lì sposta quella specifica combinazione, senza toccare le altre. Il cassetto mostra le bozze del piatto in **tutte le uscite** — cambiando uscita (o spostando una riga) il battuto non sparisce dal menu, è il chip a dire dove sta ognuna. Al primo pezzo battuto, sulla riga del piatto nel menu compare **l'uscita di destinazione** («2ª uscita», «Bar»): un tocco sul chip apre il selettore e sposta il battuto di quel piatto in un'altra uscita, senza cercare la riga in comanda.
- **«Aggiungi un altro»** nel foglio varianti (palmare e Cassa): batte la riga com'è configurata e azzera il foglio senza chiuderlo, col contatore delle righe battute — due bistecche con peso e cottura diversi in un'apertura sola. «Aggiungi» resta il gesto che batte e chiude.
- **Vino consigliato**: se il piatto ha vini abbinati in scheda, il foglio varianti mostra una scheda a fisarmonica «Vino consigliato», chiusa di default come i gruppi facoltativi, con la somma dei calici in comanda leggibile in testata; dentro, le righe dei vini con uno stepper − / +: il «+» batte il calice direttamente nell'uscita Bar, il conteggio è la quantità vera in comanda (sopravvive a chiudere e riaprire il foglio) e il «−» la corregge senza andare a cercare la riga. In Cassa la sezione non compare.
- **Foto del piatto, da mostrare al tavolo**: dove la scheda del piatto ha una foto, accanto al nome compare la miniatura — nella lista del palmare, nella vista compatta e a pagine, e nella griglia della Cassa. Il tocco sulla miniatura **non batte**: apre la foto a tutto schermo su fondo scuro, con solo nome e prezzo sotto — il telefono si gira verso l'ospite, un tocco ovunque chiude. I piatti senza foto restano identici a prima. Le miniature compaiono anche nei risultati della **ricerca piatti**, e la foto aperta da lì non chiude la ricerca.
...
- **Dopo l'invio si torna ai tavoli**: «Invia» e «Invia tutto» chiudono la comanda e riportano alla griglia, perché il gesto successivo è quasi sempre un altro tavolo. Prima, sulla comanda ancora aperta, un piccolo volo attraversa il bottone da sinistra a destra (circa un secondo) e dice cosa è partito: il piatto per la cucina, la bottiglia per il bar, la fetta di torta per dolci e gelati — insieme quando partono insieme. Le bozze delle altre uscite restano e tornano alla riapertura.
- **Presenza sul tavolo**: aprendo un tavolo su cui un altro palmare sta già componendo, un banner dice chi c'è («Ci sta lavorando anche Rosa · da 3′») e si aggiorna in tempo reale. Nessun blocco: si continua a lavorare — il subentro e il doppio cameriere sul tavolo grande restano possibili — ma nessuno batte più gli stessi piatti senza sapere dell'altro.
...
- **L'uscita pronta arriva al cameriere da sola**: quando la cucina spunta l'ultima riga, parte una **notifica push** ai ruoli di sala («Tavolo 40 — cucina · 2ª uscita pronta») e sul palmare suona un chime con vibrazione, anche dalla griglia tavoli o da un altro tavolo; lo stesso avviso suona sulla chiamata dal passe. Se poi il passe preme Chiama, la notifica si aggiorna invece di raddoppiare. Il suono si spegne per dispositivo dal menu ⋮ del tavolo («Avvisi sonori»); la push, con le notifiche del telefono attive, arriva anche su uno **smartwatch abbinato**.
- **Chiusura a un tocco** sul conto del tavolo: **Scontrino contanti** e **Scontrino POS** incassano l'importo pieno ed emettono il documento in un gesto; **Preconto** stampa in sala; "Incassa con la cassa" apre il pannello completo (dividi, misto, sospeso, mancia) per chi ha il permesso di cassa. Nel riepilogo del pannello ogni riga dice cosa è già pagato — «✓ pagato», «1 pagato» sui coperti divisi, «in pagamento» per una quota dal QR ancora in corso — e la conferma «Registra … e chiudi» resta sempre in vista in fondo, a tutta larghezza, sotto Correggi · Sconto · Dividi conto.
...
- Azioni: **storno riga inviata** con motivazione obbligatoria — anche **parziale** (su una riga da 2 o più si sceglie quanti pezzi stornare: la riga resta in cucina con la quantità scalata, lo scarto va a bilancio da solo) —, richiama in bozza, **trasferimento su altro tavolo**, sconto (importo o percentuale con motivazione), segnalazioni "Piatto non riuscito" / "Ingrediente finito", chiusura comanda con apertura conto.
...
- La **testata della card** mostra i **coperti** del tavolo accanto al nome (l'uscita si dimensiona a colpo d'occhio: «4 primi su 6 coperti, due lo saltano»), e **si tocca**: si apre la **comanda per intero** in un modal — tutte le uscite in ordine con varianti, note e stato (in coda, in lavorazione, pronta, servita con l'ora), l'avviso allergie in testa. Consultazione in corpo grande: le spunte restano sulle card.
...
- Nelle **uscite compresse in una riga** (uscite servite o di altre partite sulla card, uscite in arrivo, Consegnate) i piatti uguali si sommano; le righe **al peso** si sommano solo a **parità di peso**, col peso nel nome («2× Tagliata · 1 kg») — pesi diversi restano voci distinte. Anche la **barra dei piatti raggruppati** fa una card per pezzatura **e per variante**: nome del piatto sul primo rigo e varianti sul secondo («Bistecca» / «500 g · ben cotta», nella tinta delle varianti), col **totale ancora da cucinare** in un blocco scuro a sinistra e la **quota già sul fuoco** in un blocco oro pulsante a destra — leggibile da un metro, così il cuoco sa cosa iniziare senza aprire nulla.
- Il **dettaglio della card** («dove va questo piatto», dal tocco) usa la stessa grammatica in grande: due colonne allineate — «In lavorazione / chiamati» e «In arrivo / da chiamare» — con una card per tavolo e uscita: quantità nel blocco scuro, tavolo al centro, uscita a destra nel colore del suo pallino di timeline («1 Usc.», «Bar», «Dolci»; oro pulsante sul fuoco, neutro in arrivo). Un'uscita **già chiamata** ma in attesa dello scaglionamento pulsa per intero anche nella colonna in arrivo: sta per toccare a te.
...
- Pannello di incasso completo: più metodi sullo stesso conto (contanti, POS, Satispay, buoni pasto, gift card, sospeso, omaggio), resto calcolato, mancia con il suo metodo (contanti, POS o Satispay: quella in contanti entra nei contanti attesi del cassetto, alla riga «Mance in contanti», e l'esito della chiusura la riporta a parte), dividi conto — in parti uguali, per importo o **per piatti** (si spunta cosa ha preso la persona, anche metà di una riga da più pezzi, e la quota si calcola da sola; come dal QR, solo sui conti senza sconto; i piatti già coperti — da un incasso precedente o da una quota del QR — restano segnati «già pagata» e non si ripropongono, e dal QR l'ospite non può riprenderli; lo storno del movimento li libera) —, QR pay-at-table, scelta del documento (scontrino / proforma / fattura).
- **Sconto sul conto** anche in fase di incasso (percentuale o importo, con motivazione facoltativa e nome di chi lo concede — in cassa si sconta anche al volo, la motivazione resta per chi vuole lasciare traccia): a comanda chiusa lo sconto vive sul conto, si somma agli eventuali sconti di comanda, e compare come riga negativa su schermo e sul preconto stampato — così le righe tornano col totale. Non si scende mai sotto il già incassato (lì serve un rimborso) e un conto aperto a mano non si sconta: il totale lì è digitato. Uno sconto pari all'intero conto porta il totale a zero: il conto si chiude saldato senza incassare nulla, registrato come proforma (un documento fiscale da 0 € non esiste).
...
- **Scontrino digitale per l'ospite**: appena il documento è confermato, l'esito di chiusura (e la scheda del conto) mostra un **QR** — il cliente lo inquadra e ha lo scontrino sul telefono, su una pagina pubblica salvabile e stampabile, senza login. Accanto, **"Stampa copia"** manda la copia di cortesia (non fiscale, con lo stesso QR) alla stampante termica. La pagina è bilingue IT/EN (selettore in alto, lingua iniziale dal browser); la semantica fiscale resta italiana anche in inglese.
...
- **Proforma** quando lo scontrino non è previsto; annullo del documento fiscale come atto separato (il conto non si tocca). **"Stampa proforma"** manda il foglio alla termica — dall'esito di chiusura o, anche a giorni di distanza, dalla scheda del conto in Pagamenti: stesso dettaglio del preconto, titolo «PROFORMA», senza QR di pagamento.
...
- **Ogni ristorante spedisce solo col proprio mittente.** WhatsApp e SMS partono dal numero e dal nome mittente del ristorante: un locale che non ne ha ancora uno non usa quello di un altro. Gli invii a mano (messaggio dall'inbox, conferma, promemoria, link del conto e dell'acconto, preventivo del banchetto) rispondono «Messaggi non ancora attivi per questo ristorante»; quelli automatici saltano WhatsApp e SMS senza bloccare la prenotazione (l'email, se c'è, parte lo stesso). Se il link della caparra automatica non ha modo di arrivare, non si crea: la richiesta resta da confermare a mano. Nei ristoranti demo i messaggi arrivano solo ai numeri di prova abilitati dalla piattaforma.
...
- **Una prenotazione, una richiesta e basta**: la prenotazione viene presa in carico prima dell'invio, quindi non può ricevere un secondo messaggio nemmeno se qualcosa va storto durante l'invio. Il limite di una richiesta ogni 60 giorni per numero guarda i messaggi realmente usciti, non solo quelli registrati.
- **Pagina Recensioni** (voce nel gruppo Comunicazioni): il registro delle richieste inviate — chi ha ricevuto il link e su che canale, chi è stato saltato e perché (senza consenso, senza recapiti, richiesta recente), gli invii non riusciti e quelli il cui esito non è stato confermato. Se il registro non si carica, l'avviso porta un «Riprova» — prima l'unica via d'uscita era ricaricare il browser.
- Nella stessa sezione si sceglie già il **livello di risposta alle recensioni** (nessuna, bozza con approvazione, automatica per le positive, tutta automatica). La scelta viene salvata ma **non è ancora operativa**: leggere le recensioni dentro il CRM e rispondere richiede il collegamento del profilo Google, oggi in cantiere — il piano e i prerequisiti stanno in `docs/recensioni-google-fase-b-plan.md`.
...
- **Profilo** — pagina di partenza dopo il login; stile dell'interfaccia (classico con angoli morbidi, o squadrato con un raggio solo — la scelta segue l'operatore su ogni dispositivo); **lingua dell'interfaccia** (italiano o inglese, oppure «come il ristorante»: anche questa per account, su ogni dispositivo); notifiche push del dispositivo.
...
- **Barra globale data + turno** condivisa da tutte le viste di servizio. La data si sposta con le frecce di un giorno alla volta, oppure si apre il **calendario dell'app**: mese intero coi giorni della settimana per esteso, oggi cerchiato, il giorno scelto pieno, e «Oggi»/«Domani» sotto la griglia. Tendina col puntatore, foglio dal basso sul telefono; segue tema e lingua dell'app.
...
- **Notifiche uniformi con Annulla**: tutte le notifiche dell'app sono la stessa pillola scura della Lista della spesa — icona che dice cosa è successo, eventuale bottone d'azione (Annulla, Ricarica, Estendi), forma estesa con dettagli sullo stesso fondo — ancorate in basso al centro su telefono e in basso a destra su desktop; gli errori restano finché non vengono chiusi con la X. Un'azione produce un toast solo (niente doppioni locale+socket). Le azioni reversibili (spunta/eliminazione nella spesa, svuota righe non inviate in Comande) offrono Annulla per cinque secondi invece di chiedere conferma prima.
...
|---|---|---|
| 2026-09-25 | Messaggi: WhatsApp e SMS | WhatsApp e SMS partono solo col mittente del ristorante: un ristorante senza mittente proprio non spedisce più dal numero del Vecchio Frantoio. Gli invii a mano rispondono «Messaggi non ancora attivi per questo ristorante», quelli automatici saltano WhatsApp e SMS senza bloccare la prenotazione, e la caparra automatica senza un canale per il link resta da confermare a mano. Nei ristoranti demo i messaggi arrivano solo ai numeri di prova abilitati (anche il preventivo del banchetto). Negli avvisi d'errore compare la spiegazione del server, non più il codice tecnico. Tolti il vecchio canale WhatsApp Vonage (sandbox dismessa) e il test diagnostico WhatsApp. Per il Vecchio Frantoio non cambia nulla. |
...
| 2026-09-24 | HACCP | Con la lingua inglese parla inglese il registro HACCP: le cinque card con i loro contatori, temperature con limite e fuori-limite, controllo olio delle friggitrici, pulizie, ricevimento merci e abbattimento. I nomi delle attrezzature (celle, frigo, friggitrici, punti di pulizia) restano come sono: sono l'impianto del ristorante e la chiave con cui la riga finisce a database, non etichette. |
...
| 2026-09-23 | Agente vocale | Se il cliente chiede un piatto in anticipo (es. lo stinco), Sofia annota la richiesta «da verificare» nella prenotazione senza garantire che sarà disponibile. |
...
| 2026-09-21 | Dashboard | Con la lingua inglese la Dashboard è ora tutta in inglese: le quattro card in cima, il deck «Adesso in sala» con le sue quattro tile, Da confermare, Stato tavoli, Affluenza, Spesa, Sotto scorta, Personale in servizio e la timeline degli arrivi. Anche la pastiglia di pagamento e il testo dello spinner seguono la lingua. |
| 2026-09-21 | Recensioni Google | Messa in chiaro: leggere le recensioni nel CRM e rispondere (anche con l'AI) non è ancora disponibile e resta in cantiere — l'iter di autorizzazione di Google è lungo. La richiesta di recensione dopo la visita funziona e non dipende da quello. |
| 2026-09-20 | Messaggi: WhatsApp e SMS | **Le richieste di recensione spariscono dall'inbox.** Non sono una conversazione e non aspettano risposta, ma ogni invio rimetteva il thread in cima con «Ciao X! Grazie per essere stati…» come anteprima: dopo l'incidente dei reinvii del 18-19/09 erano 483 messaggi su 75 thread, con gli scambi veri sepolti sotto. Ora non compaiono né in lista né in chat — insieme alle scuse mandate per quell'incidente. Restano nel database e nello storico messaggi della prenotazione, dove si guarda se la richiesta è partita. Conferme, promemoria e link di pagamento restano visibili come prima. |
| 2026-09-20 | Agente vocale "Sofia" | Una zona chiusa non si libera: al cliente che insiste («e se fa caldo ci mettete fuori?») Sofia non promette più «se si libera vi mettiamo fuori» e non annota la preferenza per una zona chiusa — ripete una volta che quelle sale sono chiuse e prosegue. La preferenza resta annotabile quando la zona è aperta ma al completo, dove un tavolo può davvero liberarsi. Se una nota chiede comunque una zona chiusa, la prenotazione la porta in sala con la smentita accanto. |
| 2026-09-19 | Funzionalità trasversali | La data della barra globale apre il **calendario dell'app** al posto di quello del browser: stessa faccia su ogni computer, tema chiaro e scuro, giorni della settimana scritti per esteso (non più le iniziali L M M G V S D, dove tre lettere su sette si ripetono), oggi cerchiato e giorno scelto pieno, «Oggi» e «Domani» sotto la griglia. Sul telefono sale dal basso invece di stare stretto contro il bordo. Il calendario del browser era in inglese anche con l'app in italiano; questo segue la lingua scelta — e con lui il calendario del modal prenotazione e quello dei pagamenti, che sono lo stesso componente. |
| 2026-09-19 | Menu | **La testata della pagina Menu si riordina in due tendine.** «Alla carta ⌄» tiene tutto ciò che riguarda i menu — quale si guarda, la rinomina e l'eliminazione sulla riga del menu stagionale, «Nuovo menu» in fondo — con **la ricerca subito accanto**; e **«Gestisci»** tiene tutto ciò che si fa al menu: menu digitale, importa da cassa, categorie e varianti, col loro numero accanto (tendina sul desktop, foglio a scorrimento sul telefono, come l'imbuto dei filtri in Prenotazioni). Sparisce così la fila di bottoni accanto alla ricerca, e «Categorie» e «Varianti» escono dalla riga delle categorie, che resta di soli filtri: **una riga sola che scorre in orizzontale**, non più tre righe a capo. Cambia solo la disposizione — nessuna funzione aggiunta o tolta, e la griglia dei piatti resta identica. |
| 2026-09-19 | Funzionalità trasversali | Il foglio **«+» della barra in basso** (telefono) guadagna la voce **«Piatto»**, che il menu «+» del desktop aveva da sempre: sul telefono si crea un piatto da lì, non più da un bottone nella testata della pagina Menu. |
| 2026-09-19 | Recensioni | La pagina si allinea alle altre: niente più colonna stretta e centrata sul monitor largo, titolo e testi sulla scala di sempre, avvisi e bottoni sui componenti condivisi, scheletro di caricamento al posto della riga «Caricamento…», e bersagli da 44px su «Aggiorna» e «Mostra altre». L'unica aggiunta vera: un **«Riprova»** sull'avviso di errore. |
| 2026-09-19 | Asporto | **Giorno e stop salgono nella testata** sul desktop, dove prima la riga del giorno era spenta per questa pagina; sul telefono vanno sotto il titolo, con «Nuovo ordine» accanto al titolo. Nuova impostazione **«Giorno della board»** (Impostazioni → Asporto): indipendente come adesso, oppure agganciato alla data dell'app. Il default non cambia niente a chi già lo usa. |
| 2026-09-19 | Asporto | La board si allinea alle altre pagine: niente più colonna centrata a metà schermo sul monitor largo, titolo e icona via dove la barra laterale già dice dove sei, giorno col **navigatore condiviso** (prima le frecce erano da 44px e la pillola in mezzo da 36, e tornava solo a oggi senza far scegliere una data), e **«Nuovo ordine» entra nel menu «+» della testata** — sul telefono il bottone in pagina resta. Solo aspetto: non cambia cosa fa la pagina. |
| 2026-09-19 | Prenotazioni | L'**ordinamento è entrato nel pannello dei filtri** e il pulsante con le frecce sparisce dalla barra: l'imbuto apre «Ordina per» — un menu a tendina in cima — e sotto «Filtra per» con le pastiglie di sempre. Uguale su telefono, tablet e desktop; «Reimposta» riporta anche l'ordinamento al valore di partenza. |
| 2026-09-19 | Canali di prenotazione | Con la lingua inglese parla inglese la pagina dei canali: agente vocale e prenotazioni web, sospensioni programmate, giorni gestiti solo da operatore e limiti di occupazione per sala. |
...
| 2026-09-19 | Recensioni Google | La richiesta di recensione non può più essere inviata due volte alla stessa prenotazione: la riga viene presa in carico prima dell'invio e il limite dei 60 giorni per numero controlla i messaggi realmente usciti. Corretto il difetto per cui l'esito non veniva salvato e la richiesta ripartiva a ogni giro. |
...
| 2026-09-19 | Messaggi: WhatsApp e SMS; Pagamenti, conto al tavolo e cassa; Asporto; Menu & Banchetti | Anche i messaggi di pagamento parlano inglese all'ospite anglofono (stessa regola dei messaggi di prenotazione): link conto al tavolo e asporto, richiesta e conferma caparra, avviso di rimborso, email dell'acconto, cornice delle email libere e invio del preventivo banchetto (WhatsApp + email); i template WhatsApp inglesi partono solo dove esiste la variante approvata da Meta, altrimenti resta l'italiano — mai ibridi. |
| 2026-09-19 | Pagamenti, conto al tavolo e cassa; Menu & Banchetti | Le pagine pubbliche dello scontrino digitale (/scontrino) e del preventivo banchetto (/preventivo) parlano anche inglese: selettore IT/EN come su /pay, lingua iniziale dal browser, date e importi nel formato della lingua; la semantica fiscale resta italiana (l'EN nomina «documento commerciale» e Agenzia delle Entrate). |
| 2026-09-19 | Asporto | La pagina pubblica `/ordina` parla anche inglese: selettore IT/EN in testata (lingua iniziale dal browser), tutto tradotto — orari, menu, errori, conferma — con date nel formato della lingua; stesso motore e stessi file di traduzione del widget /prenota. Nei termini inglesi di /prenota la caparra non cita più l'articolo del Codice Civile. |
...
| 2026-09-14 | Comande, Cucina e Passe | Variante a pagine, «Categorie a bottoni»: le tessere adottano il design delle schede categoria dello schermo largo — tinta della categoria (lo stesso colore su palmare e desktop), icona riconosciuta dal nome e conteggio dei piatti; il pallino «c'è roba nell'uscita» sale accanto all'icona. |
| 2026-09-13 | Funzionalità trasversali | Nuovo stile dell'interfaccia «squadrato» a scelta dell'operatore (Impostazioni → Profilo → «Stile dell'interfaccia»): tre soli raggi — 6px le scatole (schede, pannelli, tessere, fogli), 8px i controlli (bottoni, campi, pastiglie, chip), 4px ciò che sta dentro una scatola. Il default resta il classico di sempre (scatole morbide, controlli a pillola); restano tondi pieni in entrambi gli stili gli avatar, i pallini di stato, le barre di avanzamento, gli interruttori e la barra di navigazione del telefono, che galleggia. La scelta è per account e segue l'operatore su ogni dispositivo; pagina pubblica di prenotazione e fogli di stampa non cambiano. |
...
| 2026-09-12 | Menu & Banchetti | Rifinitura Title Case: sigle puntate maiuscole («Salumi d.o.p.» → «Salumi D.O.P.», «Mel.» intatta) e preposizioni/articoli/congiunzioni minuscoli quando non aprono il titolo («Filetto ai Porcini», «Spaghetti all'Aglio e Olio»). |
| 2026-09-12 | Menu & Banchetti | Titoli del menu normalizzati in Title Case (piatti, categorie, varianti, ingredienti), una tantum sui dati esistenti e da qui in poi a ogni salvataggio e a ogni sync dalla cassa; le denominazioni restano sigle (DOC, DOCG, IGT, IGP, DOP, STG, AOC, AOP). |
| 2026-09-12 | Funzionalità trasversali | Tutte le notifiche vestono la pillola scura della spesa (ritirata la card bianca, anche per gli esiti con dettagli); un'eliminazione/creazione/modifica di prenotazione produce un solo toast, col nome del cliente, invece di due o tre copie. |
...
| 2026-09-11 | Comande, Cucina e Passe | Foglio varianti: prezzo vivo sul bottone di conferma («Aggiorna · 10,50 €», con varianti, peso e quantità), valore scelto in testata dei gruppi chiusi a scelta singola, via il sottotitolo «Varianti» dalla testata del foglio. |
| 2026-09-11 | Comande, Cucina e Passe | Foglio varianti ridisegnato: ogni gruppo è una scheda con testata (nome, conteggio, chevron) e opzioni dentro la scheda stessa, divise da hairline — via le pillole, il cui contenuto aperto galleggiava scollegato. |
...
| 2026-09-10 | Comande, Cucina e Passe | In comanda la singola riga si sposta col tocco lungo sulla riga (si stacca con vibrazione e anteprima; rilascio fermo = selettore): via le maniglie ⇅, che partivano da sole durante lo scroll. La maniglia ⇕ dell'uscita intera resta. |
| 2026-09-10 | Comande, Cucina e Passe | Sotto-righe delle combinazioni nel menu (palmare, lista e a pagine): visibili da sole quando c'è struttura, la sola «liscia» si svela col doppio tap o con la freccia sulla riga (che ruota, e richiude); a righe aperte la riga piatto si spoglia (via cestino e contatore — vivono sulle sotto-righe); stepper per riga, tap che apre il suo foglio e chip dell'uscita per spostare la singola combinazione. Sul palmare si aggiunge solo dal «+». |
| 2026-09-10 | Comande, Cucina e Passe | Nella pagina di categoria del menu a pagine il ritorno è la freccia piena in accent (via la parola «Categorie») — stessa grammatica della freccia indietro del tavolo. |
| 2026-09-10 | Comande, Cucina e Passe | Tocco lungo su un piatto già battuto nell'uscita in composizione: riapre la sua riga in bozza («Aggiorna») invece del foglio di battuta nuova — la variante corregge, non duplica. |
| 2026-09-10 | Comande, Cucina e Passe | Menu a pagine: con l'uscita Bar in composizione le sezioni Cucina e Dolci si attenuano, con Dolci si attenuano Cucina e Bar — attenuate, non bloccate. |
| 2026-09-10 | Comande, Cucina e Passe | Foglio di riga: uscita visibile col chip per spostarla, gruppi varianti come pillole col conteggio che si aprono a fisarmonica animata, e «Aggiungi un altro» anche riaprendo una riga dalla comanda (duplica nella stessa uscita). |
| 2026-09-10 | Comande, Cucina e Passe | La Comanda a tutta pagina del layout «a pagine» dice in testa tavolo e cameriere (via «Comanda» e conteggio righe), le due letture diventano un interruttore a icone accanto alla ×, che è piena in accent — è il gesto di ritorno della pagina. |
| 2026-09-09 | Comande, Cucina e Passe | Foglio varianti (palmare e Cassa): i gruppi facoltativi partono ripiegati in una riga col riassunto delle scelte, aperti al tocco; gli obbligatori restano sempre aperti. |
| 2026-09-09 | Comande, Cucina e Passe | Le categorie a bottoni del layout «a pagine» si dividono in sezioni Cucina / Bar / Dolci (dalle spunte del catalogo), con intestazioni leggere a icona tinta e ingresso a cascata delle tessere. |
| 2026-09-09 | Comande, Cucina e Passe | Nel layout «a pagine» la testata del tavolo è una riga sola (coperti, lente, ⋮, col Conto nel menu) e le categorie si mostrano a scelta dell'operatore: lista, o bottoni da 3 o 4 per riga (menu ⋮, preferenza per dispositivo). |
| 2026-09-09 | Comande, Cucina e Passe | Nel layout «a pagine» la griglia tavoli si sfoglia per sala: linguette delle sale in basso come in cassa (pallino dove c'è da fare), tessere in ordine di numero, ricerca su tutte le sale. |
| 2026-09-09 | Comande, Cucina e Passe | Nel layout «a pagine» la Comanda si apre a tutta pagina (angoli vivi, niente maniglia): è una sezione della barra, non un cassetto dietro il totale. |
| 2026-09-09 | Comande, Cucina e Passe | Layout «a pagine» del palmare comande a scelta in Impostazioni → Profilo: menu per pagine come sull'app di cassa (categorie → piatti), barra inferiore Tavoli · Comanda · Menu, uscita in composizione e «Segue» in basso; preferenza per account, vale su ogni palmare. |
...
| 2026-09-07 | Comande, Cucina e Passe | La barra dei piatti raggruppati passa da chip a card leggibili da un metro: nome sul primo rigo e varianti sul secondo, totale da cucinare in blocco scuro a sinistra, quota sul fuoco in blocco oro pulsante a destra. Il dettaglio «dove va» si ingrandisce nella stessa grammatica: colonne «In lavorazione / chiamati» e «In arrivo / da chiamare» allineate, uscita nel colore del pallino di timeline, card che pulsa se l'uscita è chiamata ma non ancora sul fuoco. Sul palmare il tasto per tornare ai tavoli diventa un cerchio pieno in accent. |
...
| 2026-09-07 | Comande, Cucina e Passe | L'uscita che diventa pronta in cucina manda da sola una notifica push ai ruoli di sala (arriva anche sullo smartwatch abbinato al telefono) e fa suonare il palmare — chime e vibrazione anche fuori dalla comanda aperta, pure sulla chiamata dal passe; avvisi sonori spegnibili per dispositivo dal menu ⋮ del tavolo. |
