...
export class SocketService {
  ...
  private padBroadcast(tenantId: number, tableId: number) {
    // Via emitTo come ogni broadcast: così arriva anche ai client in LAN via
    // nodo di sala. Limite noto della tappa 3: la presenza dei palmari
    // COLLEGATI AL NODO non risale (i loro orderpad:join muoiono sul nodo,
    // che non li inoltra) — in modalità ibrida la presenza è parziale.
    this.emitTo(tenantId, [this.tenantRoom(tenantId)], 'orderpad:presence', {
      table_id: tableId,
