...
// ---------------------------------------------------------------------------
// Foto degli articoli. GetArticoli porta SEMPRE le immagini nel payload
// (~14MB di XML sull'intero catalogo): getArticoliMenu le ignora perché non
// passerebbero dal socket verso il bridge (buffer 1MB). Qui si estraggono e
// si servono a LOTTI sotto quel limite; la cache evita di far serializzare
// 14MB al gestionale per ogni lotto.
// ---------------------------------------------------------------------------

let fotoCache: { at: number; map: Map<number, string> } | null = null;
const FOTO_CACHE_TTL_MS = 10 * 60_000;

/** Il contratto porta i byte nudi in base64: il mime si legge dalla firma. */
...
/** Il campo immagine dell'articolo: si provano i nomi noti del contratto
 *  (ImmagineBin/FotoBin) e in fallback qualunque stringa lunga che sembri
 *  base64 — le installazioni non sono tutte uguali e il WSDL qui non c'è. */
const fotoBase64Of = (a: Record<string, any>): string | null => {
    for (const k of ['ImmagineBin', 'FotoBin', 'Immagine', 'Foto']) {
        const v = a[k];
        if (typeof v === 'string' && v.length > 200) return v.replace(/\s+/g, '');
    }
    for (const v of Object.values(a)) {
        if (typeof v === 'string' && v.length > 2000 && /^[A-Za-z0-9+/=\s]+$/.test(v.slice(0, 4000))) {
...
export interface FotoArticoliResult {
    foto: { id: number; dataUri: string }[];
    /** Articoli richiesti che in cassa non hanno immagine. */
    senzaFoto: number[];
    /** Immagini singole oltre il budget del socket: mai spedite. */
    troppoGrandi: number[];
    /** Id non processati per budget: da richiedere al giro dopo. */
    resto: number[];
}

export async function getFotoArticoli(ids: number[], budgetBytes = 700_000): Promise<FotoArticoliResult> {
    if (!fotoCache || Date.now() - fotoCache.at > FOTO_CACHE_TTL_MS) {
        const result = (await soapCall(
            'GetArticoli',
            `<ultimaModifica>2000-01-01T00:00:00</ultimaModifica>`,
            120_000,
        )) as Record<string, any> | null;
        const entries: Record<string, any>[] = result && !isNil(result) ? (result.ContrattoArticolo ?? []) : [];
        const map = new Map<number, string>();
        for (const a of entries) {
            const id = asNumber(a.IdGestionale);
            if (id == null) continue;
            const b64 = fotoBase64Of(a);
            if (b64) map.set(id, dataUriFromBase64(b64));
        }
        fotoCache = { at: Date.now(), map };
    }
    const out: FotoArticoliResult = { foto: [], senzaFoto: [], troppoGrandi: [], resto: [] };
    let size = 0;
    for (const id of ids) {
        const uri = fotoCache.map.get(id);
        if (!uri) { out.senzaFoto.push(id); continue; }
        if (uri.length > budgetBytes) { out.troppoGrandi.push(id); continue; }
        if (size + uri.length > budgetBytes) { out.resto.push(id); continue; }
        out.foto.push({ id, dataUri: uri });
        size += uri.length;
    }
    return out;
}
