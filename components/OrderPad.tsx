...
export const OrderPad: React.FC<OrderPadProps> = ({ dishes: allDishes, menus, tables, reservations, globalDate, globalShiftFilter, onImmersive, initialTableId, onInitialTableConsumed }) => {
  ...
  // Categorie da bar (spunta «bar» in modale Categorie): i loro piatti vanno
  // dritti nell'uscita Bar alla battuta, qualunque uscita sia selezionata —
  // bibite e vini non stanno nella sequenza delle portate.
  ...
  const doVoid = async (item: OrderItem, reason: string, qty?: number) => {
    setBusy(true); setError(null);
    try {
