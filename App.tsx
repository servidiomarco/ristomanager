...
const App: React.FC = () => {
  ...
  const createMenuClusters: { label: string; Icon: React.ComponentType<{ size?: number; className?: string }>; show: boolean; run: () => void }[][] = [
    [
      ...
      { label: 'Ordine d’asporto', Icon: ShoppingBag, show: canAccessView(ViewState.ASPORTO) && hasPermission('takeaway:manage'), run: () => { setView(ViewState.ASPORTO); setAutoOpenNewTakeaway(true); } },
