...
describe('billing Stripe (Fase D3)', () => {
    ...
    it('subscription active con price voice → feature voice accesa, le altre spente', async () => {
        ...
        expect(await featuresOf(tenantId)).toEqual({ voice: true, whatsapp: false, web_booking: false, pay_at_table: false, passepartout: false, reviews: false });
    });
    ...
    it('canceled → tenant sospeso e feature spente', async () => {
        ...
        expect(await featuresOf(tenantId)).toEqual({ voice: false, whatsapp: false, web_booking: false, pay_at_table: false, passepartout: false, reviews: false });
    });
    ...
    it('guardia grandfathered: il tenant 1 (billing_status NULL) non è toccato dalla sync', async () => {
        ...
        expect(row.status).toBe('active');
        expect(await featuresOf(1)).toEqual({ voice: true, whatsapp: true, web_booking: true, pay_at_table: true, passepartout: true, reviews: true });
    });
