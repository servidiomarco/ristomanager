...
//
// Binario dello scontrino: di default 'e-receipts'; con
// OPENAPI_RECEIPTS_CHANNEL=receipts i comandi scontrino/stato/annullo
// passano dal documento commerciale online (/IT-receipts) — il ponte per la
// produzione finché Openapi non apre E-Receipts fuori sandbox.
...
// Binario scontrino: l'env decide quale percorso provare per primo; l'altro
// resta come fallback diagnostico. Il binario classico non accetta 'type'
// né 'additional_text' e vuole le aliquote senza decimali ("10", non
// "10.00") — stesso adattamento del driver.
const RECEIPT_PATHS = process.env.OPENAPI_RECEIPTS_CHANNEL === 'receipts'
    ? ['/IT-receipts', '/IT-e-receipts']
    : ['/IT-e-receipts', '/IT-receipts'];
const adaptForPath = (path, payload) => {
    if (path !== '/IT-receipts') return payload;
    const { type, additional_text, ...rest } = payload;
    return {
        ...rest,
...
async function scontrino() {
    ...
    for (const path of RECEIPT_PATHS) {
        const r = await call('POST', path, adaptForPath(path, payload));
        show(`POST ${path}`, r);
        if (r.status < 400) {
